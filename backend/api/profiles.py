...
def update_profile(profile_id: int, body: ProfileUpdate, session: Session = Depends(get_session)):
    ...
    try:
        ...
        bound_devices = session.query(Device).filter(Device.profile_id == profile_id).all()
        for device in bound_devices:
            policy_service.apply_device_policy(device, profile)
        session.commit()
    except Exception as exc:
        session.rollback()
        raise HTTPException(status_code=500, detail="Failed to update profile policy") from exc
